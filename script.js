  ...
  await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&APPID=${APPID}`)
    .then(result => {
      ...
      if (result.status >= 400) {
        document.getElementsByClassName("forecast-panel")[0].style = "display: none";
        document.getElementsByClassName("main-panel")[0].style = "display: none";
        document.getElementById("myVideo").style = "display: none";

        for (let i = 0; i <= 28; i++) {
          document.querySelector('div.contener').getElementsByClassName('data')[i].innerHTML = '-';
        }
...
function init(resultFromServer) {
  ...
  document.getElementsByClassName("forecast-panel")[0].style = "";
